...
async function sendRequest(): Promise<number> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Extract and return the value as a number
      // const randomNumber = parseFloat(data.random_number);
      // if (isNaN(randomNumber)) {
      //   throw new Error("Invalid random number received from API");
      // }
      // return randomNumber;
...
export const signAndCombineAndSendTx = async () => {
  ...
  try {
    ...
    const unsignedTransaction = {
      ...
      value: valueInWei, // ethers will handle the conversion to hex
      gasLimit: 21_000,
      gasPrice: (await ethersWallet.getGasPrice()).toHexString(),
